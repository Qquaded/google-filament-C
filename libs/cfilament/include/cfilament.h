...
typedef struct FInstanceBuffer FInstanceBuffer;
typedef struct FTextureBuilder FTextureBuilder;
typedef struct FInstanceBufferBuilder FInstanceBufferBuilder;
...
APIEXPORT void APICALL Filament_Engine_DestroyInstanceBuffer(FEngine *engine, FInstanceBuffer *buffer);
...
APIEXPORT void APICALL Filament_RenderableManager_BuilderInstances(
        FRenderableManagerBuilder *builder, size_t instanceCount);
APIEXPORT void APICALL Filament_RenderableManager_BuilderInstances2(
        FRenderableManagerBuilder *builder, size_t instanceCount, FInstanceBuffer* instanceBuffer);
...
APIEXPORT void APICALL Filament_Renderer_ReadPixels(FRenderer *renderer, FEngine* engine,
        uint32_t xoffset, uint32_t yoffset, uint32_t width, uint32_t height,
        void* buffer, size_t sizeInBytes, FPixelDataFormat format, FPixelDataType type,
        uint8_t alignment, uint32_t left, uint32_t top, uint32_t stride, FFreeBufferFn freeCallback, void* user);
...
typedef struct FAssetLoader FAssetLoader;
typedef struct FFilamentAsset FFilamentAsset;

// Simplified configuration for C wrapper
APIEXPORT FAssetLoader *APICALL Filament_AssetLoader_Create(FEngine *engine);
APIEXPORT void APICALL Filament_AssetLoader_Destroy(FAssetLoader **loader);

APIEXPORT FFilamentAsset *APICALL Filament_AssetLoader_CreateAsset(FAssetLoader *loader, const uint8_t *bytes, uint32_t byteCount);
APIEXPORT void APICALL Filament_AssetLoader_DestroyAsset(FAssetLoader *loader, const FFilamentAsset *asset);

// FilamentAsset
APIEXPORT size_t APICALL Filament_FilamentAsset_GetEntityCount(const FFilamentAsset *asset);
APIEXPORT const FEntity *APICALL Filament_FilamentAsset_GetEntities(const FFilamentAsset *asset);
APIEXPORT FEntity APICALL Filament_FilamentAsset_GetRoot(const FFilamentAsset *asset);

// Note: Additional features like fetching resources are usually required,
// but for simple cases or embedded GLBs, this might suffice.
