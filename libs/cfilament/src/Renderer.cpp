...
*/
void Filament_Renderer_ReadPixels(Renderer *renderer, Engine* engine,
    ...
    PixelBufferDescriptor desc(buffer, sizeInBytes, (driver::PixelDataFormat) format,
        (driver::PixelDataType) type, alignment, left, top, stride, freeCallback, user);

    renderer->readPixels(xoffset, yoffset, width, height, std::move(desc));
}
